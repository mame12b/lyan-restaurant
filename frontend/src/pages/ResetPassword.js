        ...
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <Paper elevation={10} sx={{ p: 5, borderRadius: 4, backgroundColor: alpha(theme.palette.background.paper, 0.98) }}>
            <Box sx={{ textAlign: 'center', mb: 4 }}>
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, type: 'spring', stiffness: 200 }}>
                <Box sx={{ width: 80, height: 80, borderRadius: '50%', background: brandGradient, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', boxShadow: `0 8px 20px ${alpha(brandColors.green, 0.35)}` }}>
                  <LockResetOutlined sx={{ fontSize: 40, color: theme.palette.primary.contrastText }} />
                </Box>
              </motion.div>
            ...
            <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <Button type="submit" fullWidth variant="contained" disabled={loading} sx={{ mt: 3, py: 1.5, borderRadius: 2, fontSize: '1rem', fontWeight: 600, textTransform: 'none', background: brandGradient, color: theme.palette.primary.contrastText, boxShadow: `0 4px 15px ${alpha(brandColors.gold, 0.35)}`, '&:hover': { background: `linear-gradient(135deg, ${brandColors.green}, ${brandColors.gold})`, boxShadow: `0 6px 20px ${alpha(brandColors.gold, 0.4)}`, transform: 'translateY(-2px)' }, '&:disabled': { background: '#ccc' }, transition: 'all 0.3s ease' }}>{loading ? 'Resetting Password...' : 'Reset Password'}</Button>
            </Box>
          </Paper>
